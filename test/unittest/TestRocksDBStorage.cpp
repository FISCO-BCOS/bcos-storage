...
>>>>>>> c7e092742c9a499be533f9f9800eba387fdfdae1
...
struct TestRocksDBStorageFixture {
  TestRocksDBStorageFixture() {
    ...
    rocksDBStorage =
        std::make_shared<RocksDBStorage>(std::unique_ptr<rocksdb::DB>(db));
    rocksDBStorage->asyncOpenTable(testTableName,
    ...
    if (!testTableInfo) {
      ...
      rocksDBStorage->asyncCreateTable(
          testTableName, "v1,v2,v3,v4,v5,v6",
  ...
  void writeReadDeleteSingleTable(size_t count) {
    ...
    // prewrite
    storage->asyncPrepare(params1, stateStorage,
    ...
    // commit
    storage->asyncCommit(
        bcos::storage::TransactionalStorageInterface::TwoPCParams(),
        ...
        [&](Error::UniquePtr error, std::vector<std::string> keys) {
          ...
          storage->asyncGetRows(
              ...
              [&](Error::UniquePtr error,
                ...
                for (size_t i = 0; i < tableEntries; ++i) {
                  BOOST_CHECK_EQUAL(entries[i]->getField("v4"),
                                    std::string("value3"));
                }
              });
        });
    ...
    cerr << "entries count=" << tableEntries << "|>>>>>>>>> commit="
         << std::chrono::duration_cast<chrono::milliseconds>(commitEnd - start)
                .count()
         << "ms|getAll="
         << std::chrono::duration_cast<chrono::milliseconds>(getEnd - commitEnd)
                .count()
         << "ms|deleteAll="
         << std::chrono::duration_cast<chrono::milliseconds>(deleteEnd - getEnd)
                .count()
         << "ms" << endl
         << endl;
  }
...
BOOST_AUTO_TEST_CASE(asyncGetRow) {
  ...
  tbb::parallel_for(
      tbb::blocked_range<size_t>(0, 1050),
                ...
                checks.push_back([i, entry]() {
                  if (i < 1000) {
                    ...
                    auto fields = std::vector<std::string>(
                        {"value_" + boost::lexical_cast<std::string>(i),
                         "value1", "value2", "value3", "value4", "value5"});
...
BOOST_AUTO_TEST_CASE(asyncGetPrimaryKeys) {
  ...
  rocksDBStorage->asyncGetPrimaryKeys(
      testTableName, std::optional<Condition const>(),
      [&](Error::UniquePtr error, std::vector<std::string> keys) {
        BOOST_CHECK_EQUAL(error.get(), nullptr);
        ...
        BOOST_CHECK_EQUAL_COLLECTIONS(sortedKeys.begin(), sortedKeys.end(),
                                      keys.begin(), keys.end());
      });

  auto tableInfo =
      std::make_shared<TableInfo>("new_table", vector<string>{"value"});
...
>>>>>>> c7e092742c9a499be533f9f9800eba387fdfdae1
  ...
  rocksDBStorage->asyncGetPrimaryKeys(
      ...
      [](Error::UniquePtr error, std::vector<std::string> keys) {
        ...
        for (size_t i = 0; i < 1000; ++i) {
          sortedKeys.emplace_back("newkey" +
                                  boost::lexical_cast<std::string>(i + 1000));
        }
        std::sort(sortedKeys.begin(), sortedKeys.end());
  ...
  rocksDBStorage->asyncGetRow(
      tableInfo->name(), "newkey" + boost::lexical_cast<std::string>(1050),
      [&](Error::UniquePtr error, std::optional<Entry> entry) {
  ...
  // check if the data is deleted
  rocksDBStorage->asyncGetPrimaryKeys(
      tableInfo->name(), std::optional<storage::Condition const>(),
      [](Error::UniquePtr error, std::vector<std::string> keys) {
        BOOST_CHECK_EQUAL(error.get(), nullptr);
        BOOST_CHECK_EQUAL(keys.size(), 0);
      });
        ...
        for (size_t i = 0; i < 1050; ++i) {
          ...
          if (i < 1000) {
            ...
            for (size_t i = 0; i < fields.size(); ++i) {
              std::visit(
                  [&](auto const &v) {
                    BOOST_CHECK_EQUAL(string((char *)v.data(), v.size()),
                                      fields[i]);
                  },
                  data[i]);
            }
...
BOOST_AUTO_TEST_CASE(asyncPrepare) {
  ...
  BOOST_CHECK_EQUAL(
      storage->createTable("table1", "value1,value2,value3").has_value(), true);
  BOOST_CHECK_EQUAL(
      storage->createTable("table2", "value1,value2,value3,value4,value5")
          .has_value(),
      true);
  ...
  rocksDBStorage->asyncPrepare(
      bcos::storage::TransactionalStorageInterface::TwoPCParams(), storage,
      [&](Error::Ptr error, uint64_t ts) {
        BOOST_CHECK_EQUAL(error.get(), nullptr);
        BOOST_CHECK_EQUAL(ts, 0);
      });
  ...
  rocksDBStorage->asyncGetPrimaryKeys(
      table1->tableInfo()->name(), std::optional<storage::Condition const>(),
      [&](Error::UniquePtr error, std::vector<std::string> keys) {
        BOOST_CHECK_EQUAL(error.get(), nullptr);
        BOOST_CHECK_EQUAL(keys.size(), 10);

        std::sort(table1Keys.begin(), table1Keys.end());
        BOOST_CHECK_EQUAL_COLLECTIONS(table1Keys.begin(), table1Keys.end(),
                                      keys.begin(), keys.end());

        rocksDBStorage->asyncGetRows(
            table1->tableInfo()->name(), table1Keys,
            [&](Error::UniquePtr error,
                std::vector<std::optional<Entry>> entries) {
              BOOST_CHECK_EQUAL(error.get(), nullptr);
  ...
  rocksDBStorage->asyncGetPrimaryKeys(
      table2->tableInfo()->name(), std::optional<storage::Condition const>(),
      [&](Error::UniquePtr error, std::vector<std::string> keys) {
        BOOST_CHECK_EQUAL(error.get(), nullptr);
        BOOST_CHECK_EQUAL(keys.size(), 10);

        std::sort(table2Keys.begin(), table2Keys.end());
        BOOST_CHECK_EQUAL_COLLECTIONS(table2Keys.begin(), table2Keys.end(),
                                      keys.begin(), keys.end());

        rocksDBStorage->asyncGetRows(
            table2->tableInfo()->name(), table2Keys,
            [&](Error::UniquePtr error,
                std::vector<std::optional<Entry>> entries) {
              BOOST_CHECK_EQUAL(error.get(), nullptr);
              BOOST_CHECK_EQUAL(entries.size(), 10);
...
BOOST_AUTO_TEST_CASE(boostSerialize) {
  ...
  boost::archive::binary_iarchive archive2(
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(forEncode.begin(), forEncode.end(),
                                forDecode.begin(), forDecode.end());
}
...
BOOST_AUTO_TEST_CASE(rocksDBiter) {
  ...
  for (uint32_t i = 0; i < 10; ++i) {
    ...
    for (size_t j = 0; j != 1000; ++j) {
      std::string key =
          *(bcos::toHexString(std::string((char *)&i, sizeof(i)))) + "_key_" +
...
BOOST_AUTO_TEST_CASE(commitAndCheck) {
  ...
  for (size_t keyIndex = 0; keyIndex < 100; ++keyIndex) {
    Entry entry;
    entry.importFields({boost::lexical_cast<std::string>(0)});
  ...
  rocksDBStorage->asyncPrepare(
      params, initState,
      [](Error::Ptr error, uint64_t) { BOOST_CHECK(!error); });
  rocksDBStorage->asyncCommit(params,
                              [](Error::Ptr error) { BOOST_CHECK(!error); });
  ...
  for (size_t i = 0; i < 1000; i += 100) {
    ...
    for (size_t keyIndex = 0; keyIndex < 100; ++keyIndex) {
      ...
      state->asyncGetRow(
          "test_table1", key,
          [&num](Error::UniquePtr error, std::optional<Entry> entry) {
            BOOST_CHECK(!error);
